...
class GuyaneCommuneFinanceAnalyzer:
    ...
    def _get_commune_config(self):
        """Retourne la configuration spécifique pour chaque commune guyanaise"""
        configs = {
            "Cayenne": {
            ...
            "Saint-Laurent-du-Maroni": {
                ...
                "budget_base": 85,
                "type": "urbaine",
                "specialites": ["frontaliere", "commerce", "bois", "transport_fluvial"]
            },
            "Kourou": {
                "population_base": 26000,
                "budget_base": 95,
                "type": "spatiale",
                "specialites": ["spatial", "recherche", "defense", "tourisme_technologique"]
            },
            "Matoury": {
                "population_base": 32000,
                "budget_base": 70,
                "type": "periurbaine",
                "specialites": ["aeroport", "zones_activites", "commerce"]
            },
            "Remire-Montjoly": {
                "population_base": 22000,
                "budget_base": 55,
                "type": "residentielle",
                "specialites": ["tourisme", "residential", "commerce", "plages"]
            },
            "Macouria": {
                "population_base": 15000,
                "budget_base": 40,
                "type": "rurale",
                "specialites": ["agriculture", "elevage", "zones_activites"]
            },
            "Maripasoula": {
                "population_base": 12000,
                "budget_base": 35,
                "type": "interieure",
                "specialites": ["orpaillage", "foret", "communautes_autochtones"]
            },
            "Mana": {
                "population_base": 11000,
                "budget_base": 32,
                "type": "cotiere",
                "specialites": ["agriculture", "elevage", "culture_riz"]
            },
    ...
    def generate_financial_data(self):
        ...
        # Données démographiques (croissance plus forte en Guyane)
        ...
        # Investissements spécifiques adaptés à la Guyane
        ...
        data['Investissement_Spatial'] = self._simulate_spatial_investment(dates)
        
        df = pd.DataFrame(data)
        
        # Ajouter des tendances spécifiques à la commune guyanaise
        self._add_municipal_trends(df)
        
        return df
    
    def _simulate_population(self, dates):
        """Simule la population de la commune (croissance plus forte en Guyane)"""
        base_population = self.config["population_base"]
        
        population = []
        for i, date in enumerate(dates):
            # Croissance démographique plus forte en Guyane
            if self.config["type"] == "urbaine":
                growth_rate = 0.028  # Croissance urbaine élevée en Guyane
            elif self.config["type"] == "spatiale":
                growth_rate = 0.032  # Croissance très forte autour du spatial
            elif self.config["type"] == "interieure":
                growth_rate = 0.035  # Croissance très forte dans l'intérieur
            else:
                growth_rate = 0.025  # Croissance rurale élevée
    ...
    def _simulate_households(self, dates):
        ...
        base_households = self.config["population_base"] / 3.2  # Taille des ménages plus grande en Guyane
        
        households = []
        for i, date in enumerate(dates):
            growth = 1 + 0.022 * i  # Croissance plus forte
    ...
    def _simulate_total_revenue(self, dates):
        ...
        for i, date in enumerate(dates):
            ...
            if self.config["type"] == "urbaine":
                growth_rate = 0.042  # Croissance plus forte en Guyane
            elif self.config["type"] == "spatiale":
                growth_rate = 0.045  # Croissance très forte pour le spatial
            else:
                growth_rate = 0.038  # Croissance rurale plus forte
                
            growth = 1 + growth_rate * i
            noise = np.random.normal(1, 0.08)  # Plus de volatilité
    ...
    def _simulate_tax_revenue(self, dates):
        ...
        base_tax = self.config["budget_base"] * 0.35  # Part plus faible des impôts locaux
        
        tax_revenue = []
        for i, date in enumerate(dates):
            growth = 1 + 0.028 * i  # Croissance plus forte
    ...
    def _simulate_state_grants(self, dates):
        """Simule les dotations de l'État (plus importantes en Guyane)"""
        base_grants = self.config["budget_base"] * 0.45  # Part plus importante des dotations
        
        grants = []
        for i, date in enumerate(dates):
            year = date.year
            # Augmentation plus forte des dotations pour les DOM
            if year >= 2010:
    ...
    def _simulate_other_revenue(self, dates):
        ...
        base_other = self.config["budget_base"] * 0.20  # Part plus faible
    ...
    def _simulate_total_expenses(self, dates):
        ...
        base_expenses = self.config["budget_base"] * 0.98  # Dépenses plus proches des recettes
        
        expenses = []
        for i, date in enumerate(dates):
            growth = 1 + 0.035 * i  # Croissance plus forte
    ...
    def _simulate_operating_expenses(self, dates):
        ...
        base_operating = self.config["budget_base"] * 0.65  # Part plus importante du fonctionnement
        
        operating = []
        for i, date in enumerate(dates):
            growth = 1 + 0.032 * i  # Croissance plus forte
            noise = np.random.normal(1, 0.06)
    ...
    def _simulate_investment_expenses(self, dates):
        ...
        base_investment = self.config["budget_base"] * 0.33  # Part plus faible
    ...
    def _simulate_debt_charges(self, dates):
        ...
        base_debt_charge = self.config["budget_base"] * 0.07  # Charges de dette plus importantes
        
        debt_charges = []
        for i, date in enumerate(dates):
            year = date.year
            if year >= 2005:
                increase = 1 + 0.01 * (year - 2005)  # Augmentation plus forte
            else:
                increase = 1
            
            noise = np.random.normal(1, 0.11)  # Plus de volatilité
    ...
    def _simulate_staff_costs(self, dates):
        ...
        base_staff = self.config["budget_base"] * 0.45  # Part plus importante du personnel
        
        staff_costs = []
        for i, date in enumerate(dates):
            growth = 1 + 0.034 * i  # Croissance plus forte
    ...
    def _simulate_gross_savings(self, dates):
        ...
        savings = []
        for i, date in enumerate(dates):
            base_saving = self.config["budget_base"] * 0.03  # Épargne plus faible
            
            year = date.year
            if year >= 2010:
                improvement = 1 + 0.008 * (year - 2010)  # Amélioration plus lente
            else:
                improvement = 1
            
            noise = np.random.normal(1, 0.15)  # Plus de volatilité
    ...
    def _simulate_total_debt(self, dates):
        ...
        base_debt = self.config["budget_base"] * 0.85  # Dette plus importante
        
        debt = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2007, 2013, 2019, 2024]:
                change = 1.25  # Augmentation plus forte
            elif year in [2009, 2015, 2021]:
                change = 0.92  # Réduction plus faible
            else:
                change = 1.0
    ...
    def _simulate_debt_ratio(self, dates):
        ...
        ratios = []
        for i, date in enumerate(dates):
            base_ratio = 0.78  # Endettement initial plus élevé
            
            year = date.year
            if year >= 2010:
                improvement = 1 - 0.01 * (year - 2010)  # Amélioration plus lente
            else:
                improvement = 1
    ...
    def _simulate_tax_rate(self, dates):
        ...
        rates = []
        for i, date in enumerate(dates):
            base_rate = 0.85  # Fiscalité initiale plus faible
            
            year = date.year
            if year >= 2010:
                increase = 1 + 0.005 * (year - 2010)  # Augmentation plus lente
            else:
                increase = 1
    ...
    def _simulate_agriculture_investment(self, dates):
        ...
        multiplier = 1.3 if "agriculture" in self.config["specialites"] else 0.9
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2005, 2010, 2015, 2020]:
                year_multiplier = 2.0  # Plans d'investissement plus importants
            else:
                year_multiplier = 1.0
    ...
    def _simulate_tourism_investment(self, dates):
        ...
        # Ajustement selon les spécialités
        multiplier = 1.6 if "tourisme" in self.config["specialites"] else 0.8
        ...
        for i, date in enumerate(dates):
            ...
            growth = 1 + 0.036 * i  # Croissance plus forte
    ...
    def _simulate_education_investment(self, dates):
        ...
        base_investment = self.config["budget_base"] * 0.05  # Part plus importante
        
        # Ajustement selon les spécialités
        multiplier = 1.4 if "education" in self.config["specialites"] else 1.0
    ...
    def _simulate_environment_investment(self, dates):
        ...
        # Ajustement selon les spécialités
        multiplier = 1.5 if "environnement" in self.config["specialites"] else 0.9
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2009, 2015, 2021]:
                year_multiplier = 2.0  # Plans plus importants pour l'environnement
            else:
                year_multiplier = 1.0
    ...
    def _simulate_spatial_investment(self, dates):
        """Simule l'investissement spatial (spécifique à la Guyane)"""
        base_investment = self.config["budget_base"] * 0.04
        
        # Ajustement selon les spécialités
        multiplier = 2.0 if "spatial" in self.config["specialites"] else 0.5
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2008, 2014, 2020]:
                year_multiplier = 2.2  # Investissements spatiaux importants
            else:
                year_multiplier = 1.0
            
            growth = 1 + 0.04 * i  # Croissance forte
    ...
    def _add_municipal_trends(self, df):
        """Ajoute des tendances municipales réalistes adaptées à la Guyane"""
        for i, row in df.iterrows():
            ...
            # Plan de développement guyanais (2006-2010)
            if 2006 <= year <= 2010:
                df.loc[i, 'Dotations_Etat'] *= 1.15
                df.loc[i, 'Investissement'] *= 1.2
            
            # Impact de la crise financière (2008-2009) - moins marqué en Guyane
            if 2008 <= year <= 2009:
                df.loc[i, 'Recettes_Totales'] *= 0.96
                df.loc[i, 'Investissement'] *= 0.85
            
            # Développement du spatial (2011-2015)
            elif 2011 <= year <= 2015:
                df.loc[i, 'Investissement_Spatial'] *= 1.4
                df.loc[i, 'Investissement_Transport'] *= 1.3
            
            # Mouvements sociaux de 2017 et plan d'urgence
            if year == 2017:
                df.loc[i, 'Dotations_Etat'] *= 1.25
                df.loc[i, 'Investissement_Social'] = df.loc[i, 'Investissement'] * 0.15
            
            # Impact de la crise COVID-19 (2020-2021)
            if 2020 <= year <= 2021:
                if year == 2020:
                    # Baisse des recettes mais soutien de l'État
                    df.loc[i, 'Autres_Recettes'] *= 0.70
                    df.loc[i, 'Dotations_Etat'] *= 1.20
            
            # Plan de relance post-COVID spécifique aux DOM (2022-2025)
            if year >= 2022:
                df.loc[i, 'Investissement'] *= 1.15
                df.loc[i, 'Investissement_Environnement'] *= 1.25
                df.loc[i, 'Investissement_Agriculture'] *= 1.2
    ...
    def create_financial_analysis(self, df):
        ...
        plt.suptitle(f'Analyse des Comptes Communaux de {self.commune} - Guyane ({self.start_year}-{self.end_year})', 
    ...
    def _plot_investments(self, df, ax):
        ...
        if 'Investissement_Spatial' in df.columns:
            ax.plot(df['Annee'], df['Investissement_Spatial'], label='Spatial', 
    ...
    def _plot_sectorial_investments(self, df, ax):
        ...
        if 'Investissement_Spatial' in df.columns:
            categories.append('Investissement_Spatial')
    ...
    def _generate_financial_insights(self, df):
        """Génère des insights analytiques adaptés à la Guyane"""
        print(f"🏛️ INSIGHTS ANALYTIQUES - Commune de {self.commune} (Guyane)")
        print("=" * 60)
        ...
        # 2. Croissance (plus forte en Guyane)
        ...
        # 3. Structure financière (spécificités guyanaises)
        ...
        # 5. Spécificités de la commune guyanaise
        print(f"\n5. 🌟 SPÉCIFICITÉS DE {self.commune.upper()} (GUYANE):")
        print(f"Type de commune: {self.config['type']}")
        print(f"Spécialités: {', '.join(self.config['specialites'])}")
        
        # 6. Événements marquants spécifiques à la Guyane
        print("\n6. 📅 ÉVÉNEMENTS MARQUANTS GUYANE:")
        print("• 2002-2005: Développement initial et renforcement des infrastructures")
        print("• 2006-2010: Plan de développement guyanais et investissements de l'État")
        print("• 2011-2015: Développement des activités spatiales et touristiques")
        print("• 2017: Mouvements sociaux et plan d'urgence pour la Guyane")
        ...
        # 7. Recommandations adaptées à la Guyane
        print("\n7. 💡 RECOMMANDATIONS STRATÉGIQUES:")
        if "spatial" in self.config["specialites"]:
            print("• Développer les retombées du spatial dans l'économie locale")
            print("• Former la main-d'œuvre aux métiers du spatial et des technologies")
        if "agriculture" in self.config["specialites"]:
            print("• Développer l'agriculture durable et les filières locales")
            print("• Valoriser les productions agricoles guyanaises")
        if "tourisme" in self.config["specialites"]:
            print("• Promouvoir l'écotourisme et le tourisme spatial")
            print("• Développer les infrastructures d'accueil touristique")
        print("• Améliorer les infrastructures de transport et de mobilité")
        print("• Préserver la biodiversité exceptionnelle de la Guyane")
        print("• Renforcer les services publics (santé, éducation, numérique)")
        print("• Développer l'économie circulaire et les filières vertes")

def main():
    """Fonction principale pour la Guyane"""
    # Liste des communes de Guyane
    communes = [
        "Awala-Yalimapo", "Mana", "Saint-Laurent-du-Maroni", "Apatou", 
        "Grand-Santi", "Papaichton", "Saül", "Maripasoula", "Camopi", 
        "Saint-Georges", "Ouanary", "Régina", "Roura", "Saint-Élie", 
        "Iracoubo", "Sinnamary", "Kourou", "Macouria", "Montsinéry-Tonnegrande", 
        "Matoury", "Cayenne", "Remire-Montjoly"
    ]
    
    print("🏛️ ANALYSE DES COMPTES COMMUNAUX DE LA GUYANE (2002-2025)")
    print("=" * 60)
    ...
    try:
        ...
        print("Choix invalide. Sélection de Cayenne par défaut.")
        commune_selectionnee = "Cayenne"
    
    # Initialiser l'analyseur
    analyzer = GuyaneCommuneFinanceAnalyzer(commune_selectionnee)
